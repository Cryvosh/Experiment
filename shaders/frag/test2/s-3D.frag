...
vec2 march(vec3 origin, vec3 direction, float nearClipDist) {
	...
	for (int steps = 0; steps < ITERATIONS; steps++) {
		float dist1 = DE_TORUS(origin + depth * direction);
		float dist2 = DE_BOX(origin + vec3(3.0,3.0,3.0) + depth * direction);
		
		float step = smin2(dist1, dist2, 2.5);
	...
	for (steps1 = 0; steps1 < ITERATIONS; steps1++) {
		float dist = DE_TORUS(origin + depth1 * direction);
		dist -= min(2.0, 1.0/(abs(depth1 - closestdepth2)+0.1)/2.0);
		
		if (dist < closest1) {
			closest1 = dist;
			closestdepth1 = depth1;
		}
		//float step = smin(closest2, dist, min(0.5, 1.0/abs(closestdepth2 - depth1)));
		//float step = smin(closest2, dist) + (abs(closestdepth2 - depth1)/1.0);
		//float step = (abs(closestdepth2 - depth1) < 2.0) ? smin(closest2, dist, 1.0) : dist;
		//float step = smin(smin(closest2, dist, 2.0), dist, abs(closestdepth2 - depth1)*0.5);
		//float step = mix(smin(closest2, dist, 1.0), dist, abs(closestdepth2 - depth1)/10.0);
		//float step = smin2(closest2, dist, 2.5) * (abs(depth1 - closestdepth2)+0.1)/2.0;
		//float step = smin2(closest2, dist, 2.5);
		float step = dist;
	...
	return vec2(min(depth1, depth2), steps1+steps2);
